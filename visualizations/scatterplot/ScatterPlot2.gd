...
export(bool) var debug setget _debug
func _debug(value):
	_ready()
	update()
...
func _set_values(value):
	values = value
	if drawn_values.size() != values.size() or animate:
		drawn_values = values
		temp_drawn_values = values
		update()
	elif tween != null and animate:
...
func _set_color(value):
	...
	if tween.is_active() or not animate:
		color = value
		update()
	elif animate:
...
func _set_size(value):
	...
	if tween.is_active() or not animate:
		...
		tween.interpolate_property(self, "size", size, value, 2)
		tween.start()
